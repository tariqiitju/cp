/*
	...
	*ComputeSignedArea (of simple polygon)
	*ComputeCentroid	(of simple polygon)
	*IsSimple			(polygon)
	*cir_union (n*n)
	:Dependencies
		*dis2(a,b)			//squired distance [not sure , could be distance]
		*RotateCCW90(p)		//as name suggest [not big deal]
		*ProjectPointSegment(a,b,c)	//project c on line(a,b) [or segment (a,b)??] use closest point from 3d geo
...
{
    double x,y;
    PT(double a=0.0,double b=0.0){x=a;y=b;}
    PT operator+(const PT&a)const{return PT(x+a.x,y+a.y);}
    PT operator-(const PT&a)const{return PT(x-a.x,y-a.y);}
    PT operator*(const double&a)const{return PT(x*a,y*a);}
    PT operator/(const double&a)const{return PT(x/a,y/a);}
    double operator*(const PT&a)const{return x*a.y-y*a.x;}
    double operator/(const PT&a)const{return sqrt((a.x-x)*(a.x-x)+(a.y-y)*(a.y-y));}
